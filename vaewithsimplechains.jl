...
# ╔═╡ 128cfbe9-3bbb-4e0c-b082-ac847caa61ed
using SimpleChains

# ╔═╡ 0fd779eb-e4bf-40a0-8498-9b0a6780ddec
using Distributions

# ╔═╡ b835bd69-7602-4a76-ac07-7197a74b1109
# Define the encoder network
function build_encoder(input_dim, encoding_dim)
    model = SimpleChain(
        TurboDense(input_dim, 512, relu),
        TurboDense(512, encoding_dim * 2)
    )
    return model
end

# ╔═╡ 84a2ee47-6dae-454a-9b16-bcf055afa15a
# Define the decoder network
function build_decoder(input_dim, encoding_dim)
    model = Chain(
        Dense(encoding_dim, 512, relu),
        Dense(512, input_dim)
    )
    return model
end

# ╔═╡ c2554257-66ea-475c-bb18-a3ab7fae42ae
begin
# Define the VAE model as a parametric struct
struct VAE{I, E, O}
    encoder::Chain
    decoder::Chain
    input_dim::I
...
function VAE(input_dim::Int, encoding_dim::Int, output_dim::Int)
    encoder = build_encoder(input_dim, encoding_dim)
    decoder = build_decoder(input_dim, encoding_dim)
    return VAE(encoder, decoder, input_dim, encoding_dim, output_dim)
end
function (m::VAE)(x)
    mean, log_var = SimpleChains.split(m.encoder(x), 2, 2)
    std = log_var .^ 0.5
    z = mean + randn(size(std)) .* std
    reconstructed = m.decoder(z)
    return reconstructed, mean, log_var
end
end

# ╔═╡ 30818880-a5cc-490c-89de-2daff737d229
# Define the reconstruction loss
function reconstruction_loss(x, reconstructed_x, mean, log_var)
    BCE = SimpleChains.Flux.crossentropy(x, reconstructed_x)
    KLD = -0.5 * SimpleChains.sum(1.0 .+ log_var - mean .^ 2 - log_var .^ 2)
    return BCE + KLD
end

# ╔═╡ 607dbccc-33cb-47f5-ab2e-a5ca8bc50398
# Train the VAE
function train_VAE(VAE, data, epochs)
    optimizer = Adam(0.001)
    for i in 1:epochs
        SimpleChains.train!(reconstruction_loss, VAE, data, optimizer)
        if i % 10 == 0
            println("Epoch ", i, " Loss: ", SimpleChains.loss(VAE, data))
        end
    end
end

# ╔═╡ f49d3620-e07f-4370-be93-cc7a33f734bb
# Initialize the VAE model
input_dim = 28 * 28

# ╔═╡ 66c8faab-7b28-4bf6-8ee4-e23fe5b05fe4
encoding_dim = 32

# ╔═╡ e0879be6-7923-452f-b0b0-eb366932a525
output_dim = input_dim

# ╔═╡ 415cda15-c5d9-442c-a4e6-46e1ee5b9ce8
vae = VAE(input_dim, encoding_dim, output_dim)

# ╔═╡ 6ade802a-aec4-4e53-8e8c-52f2ed162674
# Train the VAE
train_VAE(vae, SimpleChains.Data.mnist(), epochs=2)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "b1ece64b54b04b5e9be0278eb1bd1407a117f1e8"
...
git-tree-sha1 = "d2032df5b000bfa871e1a6a4112a399540d4f935"
