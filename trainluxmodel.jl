...
begin
    ...
    using LossFunctions
    using Flux: logitbinarycrossentropy, unstack
...
# ╔═╡ 740b62eb-43fd-46d6-b719-d15332d9ee8b
begin
    struct LuxVaeModel{L,C} <:
           Lux.AbstractExplicitContainerLayer{(:encoder, :decoder)}
        encoder::L
        decoder::C
    end
	
    function (s::LuxVaeModel)(rng, x::AbstractArray{T,4}, ps::NamedTuple,
        st::NamedTuple) where {T}
        (μ, logvar), newencoderstate = Lux.apply(s.encoder, x, ps.encoder, st.encoder)
        randcoeffs = randn(rng, Float32, size(logvar))
        z = μ .+ randcoeffs .* exp.(0.5f0 .* logvar)
        output, newdecoderstate = Lux.apply(s.decoder, z, ps.decoder, st.decoder)
        return (output, (encoder=newencoderstate, decoder=newdecoderstate))
    end
	
	function (s::LuxVaeModel)(rng, x::AbstractArray{T,4}, ps::NamedTuple,
        st::NamedTuple, n::Integer) where {T}
        (μ, logvar), newencoderstate = Lux.apply(s.encoder, x, ps.encoder, st.encoder)
			randcoeffs = 0
			for i in 1:n
				randcoeffs += randn(rng, Float32, size(logvar))
			end
        	randcoeffs = randcoeffs/n
        	z = μ .+ randcoeffs .* exp.(0.5f0 .* logvar)
        	output, newdecoderstate = Lux.apply(s.decoder, z, ps.decoder, st.decoder)
        	return (output, (encoder=newencoderstate, decoder=newdecoderstate))
    end
...
# ╔═╡ fafcfae5-36e4-47a9-a0b7-41ee44176f4c
lastdepth = 64

# ╔═╡ 2f08af66-bf7f-4cd2-8690-63894d1c5b7c
hiddendense = 64
...
# ╔═╡ ce1e7d84-7856-42f6-b8bf-7ab6de44a891
initdata = MLDatasets.MNIST(:train).features[:, :, 1:3];
...
function vaeloss(rng, x, model, ps, st)
    (μ, logvar), newencoderstate = Lux.apply(model.encoder, x, ps.encoder, st.encoder)
    randcoeffs = randn(rng, Float32, size(logvar))
    z = μ .+ randcoeffs .* exp.(0.5f0 .* logvar)
...
function trainvaemodel(lossfn, fullmodel, modelps, modelstate, dataloader, opt_state)
    ...
    @progress for i in 1:20
        ...
        for data in dataloader
            ...
            opt_state, modelps = Optimisers.update(opt_state, modelps, grads)
        end
    end
    return (modelps, opt_state, modelstate)
end
...
# ╔═╡ cf517c68-7f01-4d33-85d7-8c36e7d1a592
@bind showdata Radio(["recovereddata", "initialmodelrecovered"])

# ╔═╡ f762c485-ff6f-406b-904c-0bfb3e35de8e
@bind imgindex PlutoUI.Slider(1:size(dataforloader,4))

# ╔═╡ 37fdcd0b-a6a2-4884-b08c-6aebaab2a735
begin
	f = Figure(backgroundcolor=:lightgrey)
	ax1 = Axis(f[1,1], aspect=1, title="Original")
	plotimage(ax1, dataforloader[:,:,:,imgindex])
	ax2 = Axis(f[1,2], aspect=1, title = "Recovered")
...
git-tree-sha1 = "73fd67f0e4820bdae17e14b4e092dde542f57aab"
...
git-tree-sha1 = "eeca6bf441fcd43a370777a6c3a2b5ae17434099"
...
git-tree-sha1 = "34d9708958ef04bf2f5430f2231449620dd0acf7"
